...
export class CartPageComponent implements OnInit, OnChanges {
    ...
    tempCartId: number | undefined;
    confirmDeleteProduct(cartId: number | undefined) {
      this.tempCartId = cartId;
      this.showConfirmationModal = true;
  }
  cancelDelete() {
    this.showConfirmationModal = false;
    // Reset biến tạm thời
    this.tempCartId = undefined;
}
deleteFromCart() {
  if (this.tempCartId) {
      this.removeToCart(this.tempCartId);
      this.showConfirmationModal = false;
      // Reset biến tạm thời
      this.tempCartId = undefined;
  }
...
this.calculateDiscountFromVoucher();
 ...
 checkout() {
  ...
  showConfirmVoucherModal: boolean = false;
  
  cancelClearSelectedVoucher() {
    this.showConfirmVoucherModal = false; // Ẩn modal xác nhận nếu người dùng hủy bỏ việc bỏ sử dụng voucher
  }
  confirmClearSelectedVoucher() {
    this.selectedVoucher = ''; // Xóa voucher được chọn
    this.calculateDiscountFromVoucher(); // Tính lại giảm giá từ voucher
    this.calculateTotal(); // Tính lại tổng cộng
    this.showConfirmVoucherModal = false; // Ẩn modal xác nhận sau khi đã xác nhận bỏ sử dụng voucher
  }
  clearSelectedVoucher() {
    this.showConfirmVoucherModal = true;
